...
{{-- {{dd($field)}} --}}
    ...
    <div class="col-md-12 well repeatable-element repeatable-table row m-1 p-2" data-repeatable-identifier="{{ $field['name'] }}">
      @if (isset($field['subfields']) && is_array($field['subfields']) && count($field['subfields']))
        ...
        @foreach($field['subfields'] as $subfield)
          ...
          @php
            ...
            if ($subfield['type'] == 'relationship') {
                    ...
                    // $fieldViewPath = $fieldViewNamespace . '.' . $subfield['type'];
                        $fieldViewPath = $fieldViewNamespace . '.' . $subfield['type'].'_table';
             }else{
                $fieldViewNamespace = $subfield['view_namespace'] ?? 'crud::fields';
                $fieldViewPath = $fieldViewNamespace.'.'.$subfield['type'];
             }
          ...
          @endphp
          @include($fieldViewPath, ['field' => $subfield, 'entity_options' => $field['options']])
        @endforeach
