@php
    ...
    if (!isset($entity_options)) {
            $field['options'] = $connected_entity::all()->pluck($field['attribute'],$connected_entity_key_name);
        } else {
            // dd($entity_options);
            $field['options'] = call_user_func($entity_options, $field['model']::query())->pluck($field['attribute'],$connected_entity_key_name);
    }
...
@include('crud::fields.inc.wrapper_start_table')
    {{-- <label>Item</label> --}}
